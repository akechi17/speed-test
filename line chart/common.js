class Chart {
  ...
  drawLabels() {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      this.ctx.fillRect(this.padding - this.yAxis.pointWidth, y, this.yAxis.pointWidth, 2);
    ...
    this.data.unsortedValues.forEach((data, index) => {
      ...
      this.ctx.fillRect(x, this.canvasEl.height - this.padding, 2, this.xAxis.pointWidth);

      this.ctx.fillText(index.toString(), x - 5, this.canvasEl.height - this.padding + 25);

      //   Untuk line merah
      this.ctx.strokeStyle = "red";
      const lineY = Math.floor((this.canvasEl.height - this.padding * 2) * ((this.data.highest - data) / this.data.highest) + (this.padding * 3) / 2 + 4);
